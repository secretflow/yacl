...
#include "yasl/mpctools/ot/block.h"
...
std::vector<uint128_t> GenerateRandomUint128(size_t n) {
  ...
  for (size_t i = 0; i < n; i++) {
    rand_vec[i] = yasl::MakeUint128(random(), random());
  }
  return rand_vec;
}

std::vector<yasl::block> GenerateRandomBlock(size_t n) {
  std::random_device rd;
  std::seed_seq seed({rd(), rd(), rd(), rd(), rd(), rd(), rd(), rd()});
  std::mt19937 random = std::mt19937(seed);

  std::vector<yasl::block> rand_vec(n + 1);
  for (size_t i = 0; i < n; i++) {
    rand_vec[i] = yasl::block(yasl::MakeUint128(random(), random()));
  }
  return rand_vec;
}
}  // namespace
...
static void BM_BlockMoveMaskEpi8(benchmark::State& state) {
  for (auto _ : state) {
    ...
    std::vector<yasl::block> a = GenerateRandomBlock(n);
    std::vector<int> c(n);
...
static void BM_Uint128MoveMaskEpi8(benchmark::State& state) {
  for (auto _ : state) {
    ...
    state.ResumeTiming();
    for (size_t i = 0; i < n; i++) {
      benchmark::DoNotOptimize(c[i] = yasl::block(a[i]).movemask_epi8());
    }
  }
}
...
static void BM_BlockAdd(benchmark::State& state) {
  for (auto _ : state) {
    ...
    size_t n = state.range(0);
    std::vector<yasl::block> a = GenerateRandomBlock(n);
    std::vector<yasl::block> b = GenerateRandomBlock(n);
    std::vector<yasl::block> c(n);
...
static void BM_BlockXor(benchmark::State& state) {
  for (auto _ : state) {
    ...
    std::vector<yasl::block> a = GenerateRandomBlock(batch_size);
    std::vector<yasl::block> b = GenerateRandomBlock(batch_size);
...
namespace {
std::vector<std::vector<yasl::block>> GenerateMatrixBlock(size_t n) {
  std::random_device rd;
  std::seed_seq seed({rd(), rd(), rd(), rd(), rd(), rd(), rd(), rd()});
  std::mt19937 random = std::mt19937(seed);

  std::vector<std::vector<yasl::block>> a(n);

  for (size_t i = 0; i < n; ++i) {
    a[i].resize(n);
    for (size_t j = 0; j < n; ++j) {
      a[i][j] = yasl::block(yasl::MakeUint128(random(), random()));
    }
  }
  return a;
}
...
std::vector<std::vector<uint128_t>> GenerateMatrixUint128(size_t n) {
  ...
  for (size_t i = 0; i < n; ++i) {
    a[i].resize(n);
    for (size_t j = 0; j < n; ++j) {
      a[i][j] = yasl::MakeUint128(random(), random());
    }
  }
  return a;
}
...
static void BM_BlockMatrixXor(benchmark::State& state) {
  for (auto _ : state) {
    ...
    std::vector<std::vector<yasl::block>> a, b, c(n);
