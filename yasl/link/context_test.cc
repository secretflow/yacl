...
#include "yasl/link/transport/channel_mem.h"

namespace yasl::link::test {
...
ACTION(ThrowNetworkErrorException) { throw ::yasl::NetworkError(); }
...
TEST_F(ContextConnectToMeshTest, ThrowExceptionIfNetworkError) {
  ...
  // WHEN THEN
  EXPECT_THROW(ctx.ConnectToMesh(), ::yasl::RuntimeError);
}
