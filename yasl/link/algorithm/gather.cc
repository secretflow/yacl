...
#include "yasl/link/algorithm/gather.h"
...
std::vector<std::vector<Buffer>> GatherVectorImpl(
  ...
  if (inputs_size == 1) {
    ...
    if (root == ctx->Rank()) {
      YASL_ENFORCE(all_outputs_packed.size() == ctx->WorldSize());
      ...
      for (size_t rank = 0; rank < all_outputs_packed.size(); ++rank) {
        ...
        YASL_ENFORCE(outputs_i.size() == inputs_size);
      ...
      YASL_ENFORCE(all_outputs_packed.empty());
    }
  }
