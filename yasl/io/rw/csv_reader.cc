...
void CsvReader::Init() {
  ...
  YASL_ENFORCE(schema.feature_names.size() == schema.feature_types.size());
  const size_t f_size = schema.feature_names.size();
  // col reader do not support empty feature.
  YASL_ENFORCE(options_.column_reader == false || f_size > 0);
...
void CsvReader::ParseHeader() {
  ...
  YASL_ENFORCE(NextLine(&headers), "Can't get header from file '{}'",
               in_->GetName());
...
void CsvReader::UpdateRowMap() {
  ...
  YASL_ENFORCE(!options_.column_reader);
...
void CsvReader::BuildMmapFiles() {
  ...
  while (NextLine(&fields)) {
    ...
    for (size_t i = 0; i < selected_features_.size(); i++) {
      ...
      switch (type) {
        ...
        default:
          YASL_THROW("unknow Schema::type {}", type);
      }
    }
  }
  total_rows_ = row_count;
  {
    ...
    std::optional<yasl::Exception> ope;
    for (auto& os : oss) {
      try {
        os->Close();
...
bool CsvReader::Next(ColumnVectorBatch* data) {
  YASL_ENFORCE(inited_, "Please Call Init before use reader");
  data->Clear();
  if (options_.column_reader) {
    return NextCol(data);
  } else {
...
bool CsvReader::NextCol(ColumnVectorBatch* data) {
  YASL_ENFORCE(total_rows_ != kUnknowTotalRow);
  ...
  switch (type) {
    case Schema::STRING: {
      ...
      size_t pos = 0;
      while (pos < mmap_size) {
        YASL_ENFORCE(pos + sizeof(uint32_t) <= mmap_size);
        uint32_t len;
        std::memcpy(&len, mmap_data + pos, sizeof(uint32_t));
        pos += sizeof(uint32_t);

        YASL_ENFORCE(pos + len <= mmap_size);
        const char* str = mmap_data + pos;
        pos += len;

        col.emplace_back(str, len);
      }
      YASL_ENFORCE(col.size() == total_rows_);
    ...
    case Schema::FLOAT: {
      ...
      col.reserve(total_rows_);
      YASL_ENFORCE(mmap_size == total_rows_ * sizeof(float));
    ...
    case Schema::DOUBLE: {
      ...
      col.reserve(total_rows_);
      YASL_ENFORCE(mmap_size == total_rows_ * sizeof(double));
...
void CsvReader::Seek(size_t index) {
  ...
  if (options_.column_reader) {
    ...
    YASL_ENFORCE(current_index_ == index,
                 "seek for row out of range, try {} max {}", index,
                 current_index_);
  }
}
...
std::unique_ptr<Reader> CsvReader::Spawn() {
  YASL_ENFORCE(inited_, "CAN NOT Spawn before init");
  auto in = in_->Spawn();
  YASL_ENFORCE(in_->Tellg() == in->Tellg());
