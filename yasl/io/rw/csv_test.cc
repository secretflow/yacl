...
u2, 22 , 23 , 24 , 25 , 0
  ...
  {  // reader
    ...
    EXPECT_EQ(reader.Tell(), 0);
    EXPECT_THROW(reader.Tellg(), yasl::Exception);
...
u2, 22 , 23 , 24 , 25 , 0
  ...
  {  // error 1
    ...
    CsvReader reader(r_ops, std::move(in));
    EXPECT_THROW(reader.Init(), yasl::ArgumentError);
  }
}
...
u2, 22 , 23 , 24 , 25 , 0
    ...
    CsvReader reader(r_ops, std::move(in));
    EXPECT_THROW(reader.Init(), yasl::InvalidFormat);
  }
  {  // Repeated fields
...
u2, 22 , 23 , 24 , 25 , 0
    ...
    EXPECT_THROW(reader.Next(&data), yasl::InvalidFormat);
  }
