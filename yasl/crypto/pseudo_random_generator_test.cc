...
TEST(PseudoRandomGenerator, DeterministicWithSameSeedSm4) {
  PseudoRandomGenerator<int> prg1(kKey1, PRG_MODE::KSm4Ecb);
  PseudoRandomGenerator<int> prg2(kKey1, PRG_MODE::KSm4Ecb);
  for (int i = 0; i < 128; ++i) {
    EXPECT_EQ(prg1(), prg2());
    EXPECT_EQ(prg1(), prg2());
  }
...
TEST(PseudoRandomGenerator, DeterministicWithDifferentSeedSm4) {
  PseudoRandomGenerator<int> prg1(kKey1, PRG_MODE::KSm4Ecb);
  PseudoRandomGenerator<int> prg2(kKey2, PRG_MODE::KSm4Ecb);
  for (int i = 0; i < 128; ++i) {
    EXPECT_NE(prg1(), prg2());
    EXPECT_NE(prg1(), prg2());
  }
...
TEST(PseudoRandomCtrDrbg, BooleanWorks) {
  // GIVEN
  PseudoRandomGenerator<bool> prg(kKey1, PRG_MODE::kNistAesCtrDrbg);
  // WHEN
  std::array<int, 2> counts = {0, 0};
  const int kNumCalls = 10000;
  for (int i = 0; i < kNumCalls; ++i) {
...
TEST(PseudoRandomCtrDrbg, BuiltinScalarsWorks) {
  {
    // GIVEN
    PseudoRandomGenerator<int> prg(kKey1, PRG_MODE::kNistAesCtrDrbg);
    // WHEN
    int a = prg();
    int b = prg();
    // THEN
  ...
  {
    // GIVEN
    PseudoRandomGenerator<double> prg(kKey1, PRG_MODE::kNistAesCtrDrbg);
    // WHEN
    double a = prg();
    double b = prg();
    // THEN
  ...
  {
    // GIVEN
    PseudoRandomGenerator<uint64_t> prg(kKey1, PRG_MODE::kNistAesCtrDrbg);
    // WHEN
    uint64_t a = prg();
    uint64_t b = prg();
    // THEN
  ...
  {
    // GIVEN
    PseudoRandomGenerator<uint128_t> prg(kKey1, PRG_MODE::kNistAesCtrDrbg);
    // WHEN
    uint128_t a = prg();
    uint128_t b = prg();
    // THEN
...
TEST(PseudoRandomCtrDrbg, WorksForCustomizedStruct) {
  // GIVEN
  PseudoRandomGenerator<Foo> prg(kKey1, PRG_MODE::kNistAesCtrDrbg);
  int ncalls = 3 * decltype(prg)::BatchSize() + 13;
  Foo last = prg();
  for (int i = 0; i < ncalls; ++i) {
    // WHEN
...
TEST(PseudoRandomCtrDrbg, DeterministicWithSameSeed) {
  PseudoRandomGenerator<int> prg1(kKey1, PRG_MODE::kNistAesCtrDrbg);
  PseudoRandomGenerator<int> prg2(kKey1, PRG_MODE::kNistAesCtrDrbg);
  for (int i = 0; i < 128; ++i) {
    EXPECT_NE(prg1(), prg2());
    EXPECT_NE(prg1(), prg2());
  }
...
TEST(PseudoRandomCtrDrbg, DeterministicWithDifferentSeed) {
  PseudoRandomGenerator<int> prg1(kKey1, PRG_MODE::kNistAesCtrDrbg);
  PseudoRandomGenerator<int> prg2(kKey2, PRG_MODE::kNistAesCtrDrbg);
  for (int i = 0; i < 128; ++i) {
    EXPECT_NE(prg1(), prg2());
    EXPECT_NE(prg1(), prg2());
  }
...
TEST(PseudoRandomSm4Drbg, BooleanWorks) {
  // GIVEN
  PseudoRandomGenerator<bool> prg(kKey1, PRG_MODE::kGmSm4CtrDrbg);
  // WHEN
  std::array<int, 2> counts = {0, 0};
  const int kNumCalls = 10000;
  for (int i = 0; i < kNumCalls; ++i) {
...
TEST(PseudoRandomSm4Drbg, BuiltinScalarsWorks) {
  {
    // GIVEN
    PseudoRandomGenerator<int> prg(kKey1, PRG_MODE::kGmSm4CtrDrbg);
    // WHEN
    int a = prg();
    int b = prg();
    // THEN
  ...
  {
    // GIVEN
    PseudoRandomGenerator<double> prg(kKey1, PRG_MODE::kGmSm4CtrDrbg);
    // WHEN
    double a = prg();
    double b = prg();
    // THEN
  ...
  {
    // GIVEN
    PseudoRandomGenerator<uint64_t> prg(kKey1, PRG_MODE::kGmSm4CtrDrbg);
    // WHEN
    uint64_t a = prg();
    uint64_t b = prg();
    // THEN
  ...
  {
    // GIVEN
    PseudoRandomGenerator<uint128_t> prg(kKey1, PRG_MODE::kGmSm4CtrDrbg);
    // WHEN
    uint128_t a = prg();
    uint128_t b = prg();
    // THEN
...
TEST(PseudoRandomSm4Drbg, WorksForCustomizedStruct) {
  // GIVEN
  PseudoRandomGenerator<Foo> prg(kKey1, PRG_MODE::kGmSm4CtrDrbg);
  int ncalls = 3 * decltype(prg)::BatchSize() + 13;
  Foo last = prg();
  for (int i = 0; i < ncalls; ++i) {
    // WHEN
...
TEST(PseudoRandomSm4Drbg, DeterministicWithSameSeed) {
  PseudoRandomGenerator<int> prg1(kKey1, PRG_MODE::kGmSm4CtrDrbg);
  PseudoRandomGenerator<int> prg2(kKey1, PRG_MODE::kGmSm4CtrDrbg);
  for (int i = 0; i < 128; ++i) {
    EXPECT_NE(prg1(), prg2());
    EXPECT_NE(prg1(), prg2());
  }
...
TEST(PseudoRandomSm4Drbg, DeterministicWithDifferentSeed) {
  PseudoRandomGenerator<int> prg1(kKey1, PRG_MODE::kGmSm4CtrDrbg);
  PseudoRandomGenerator<int> prg2(kKey2, PRG_MODE::kGmSm4CtrDrbg);
  for (int i = 0; i < 128; ++i) {
    EXPECT_NE(prg1(), prg2());
    EXPECT_NE(prg1(), prg2());
  }
