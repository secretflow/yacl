...
std::vector<uint8_t> Sm4MteEncrypt(ByteContainerView key, ByteContainerView iv,
  ...
  YASL_ENFORCE_GE(iv_key_hash.size(), HMAC_KEY_SIZE);
...
std::vector<uint8_t> Sm4MteDecrypt(ByteContainerView key, ByteContainerView iv,
  ...
  YASL_ENFORCE_GT(hmac_plaintext.size(), HMAC_SIZE);
  ...
  // Step 3. check hmac
  YASL_ENFORCE_EQ(hmac_actual.size(), hmac_from_cipher.size());
  YASL_ENFORCE(std::equal(hmac_actual.begin(), hmac_actual.end(),
