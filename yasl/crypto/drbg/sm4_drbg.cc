...
#include "yasl/crypto/drbg/sm4_drbg.h"
...
std::string Sm4Drbg::RngDf(ByteContainerView seed_material) {
  ...
  *p++ = (unsigned char)((2 * kBlockSize) & 0xff);
  ...
  while (ret.length() < seed_length_) {
    ...
    YASL_ENFORCE(EVP_CipherUpdate(enc_ctx.get(), (unsigned char*)cipher.data(),
...
std::array<uint8_t, kBlockSize> Sm4Drbg::CbcMac(
  ...
  for (size_t idx = 0; idx < n; ++idx) {
    ...
    YASL_ENFORCE(EVP_CipherUpdate(ctx_cbc.get(), chaining_value.data(),
...
void Sm4Drbg::RngUpdate() {
  ...
  YASL_ENFORCE(EVP_CipherUpdate(ecb_ctx_.get(), enc_out.data(), &enc_out_len,
                                enc_in.data(), enc_in.size()));
  YASL_ENFORCE(enc_out_len == enc_in.size());
...
std::vector<uint8_t> Sm4Drbg::Generate(
  ...
  YASL_ENFORCE(rand_length <= kBlockSize);
...
void Sm4Drbg::FillRandomBytes(absl::Span<uint8_t> out) {
  ...
  if (nbytes > 0) {
    ...
    for (size_t idx = 0; idx < block_size; ++idx) {
      ...
      YASL_ENFORCE(rand_buf.size() == current_size);
