...
#include "yasl/crypto/drbg/nist_aes_drbg.h"
...
size_t GetEntropy(RAND_DRBG *drbg, unsigned char **pout, int entropy_bits,
  ...
  YASL_ENFORCE((size_t)entropy_bytes == entropy_buffer.length());
...
std::vector<uint8_t> NistAesDrbg::Generate(
  ...
  while (out_len < rand_len) {
    ...
    YASL_ENFORCE(RAND_DRBG_generate(drbg_.get(), random_buf.data() + out_len,
...
void NistAesDrbg::Instantiate(uint128_t personal_string) {
  if (personal_string == 0) {
    YASL_ENFORCE(RAND_DRBG_instantiate(drbg_.get(), nullptr, 0));
  } else {
    YASL_ENFORCE(RAND_DRBG_instantiate(drbg_.get(),
