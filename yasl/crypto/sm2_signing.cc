...
#include "yasl/crypto/sm2_signing.h"
...
std::vector<uint8_t> Sm2Signer::Sign(ByteContainerView message,
  ...
  YASL_ENFORCE_GT(EVP_DigestSignUpdate(m_ctx, message.data(), message.size()),
                  0);
  ...
  size_t sig_len;
  YASL_ENFORCE_GT(EVP_DigestSignFinal(m_ctx, nullptr, &sig_len), 0);
  std::vector<uint8_t> signature(sig_len);
  YASL_ENFORCE_GT(EVP_DigestSignFinal(m_ctx, signature.data(), &sig_len), 0);
...
std::unique_ptr<Sm2Verifier> Sm2Verifier::CreateFromOct(
  ...
  YASL_ENFORCE(nullptr != group);
  ON_SCOPE_EXIT([&] { EC_GROUP_free(group); });

  EC_POINT* ec_pub_key_pt = EC_POINT_new(group);
  YASL_ENFORCE(nullptr != ec_pub_key_pt);
  ON_SCOPE_EXIT([&] { EC_POINT_free(ec_pub_key_pt); });
  YASL_ENFORCE(EC_POINT_oct2point(group, ec_pub_key_pt, sm2_oct.data(),
                                  sm2_oct.size(), nullptr));

  EC_KEY* ec_key = EC_KEY_new();
  YASL_ENFORCE(nullptr != ec_key);
  ON_SCOPE_EXIT([&] { EC_KEY_free(ec_key); });
  YASL_ENFORCE(EC_KEY_set_group(ec_key, group));
  YASL_ENFORCE(EC_KEY_set_public_key(ec_key, ec_pub_key_pt));
  EVP_PKEY* pk = EVP_PKEY_new();
  YASL_ENFORCE(nullptr != pk);
  UniquePkey unique_pk(pk, ::EVP_PKEY_free);
  YASL_ENFORCE(EVP_PKEY_set1_EC_KEY(unique_pk.get(), ec_key));
  YASL_ENFORCE(EVP_PKEY_set_alias_type(unique_pk.get(), EVP_PKEY_SM2));
...
void Sm2Verifier::Verify(ByteContainerView message, ByteContainerView signature,
  ...
  YASL_ENFORCE_GT(EVP_DigestVerifyUpdate(m_ctx, message.data(), message.size()),
                  0);
  YASL_ENFORCE_GT(
