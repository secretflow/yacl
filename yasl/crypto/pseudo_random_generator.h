...
#include "yasl/crypto/drbg/drbg.h"
...
class PseudoRandomGenerator {
 public:
  ...
  explicit PseudoRandomGenerator(uint128_t seed = 0,
                                 PRG_MODE prg_mode = PRG_MODE::kAesEcb)
      : prg_mode_(prg_mode) {
    SetSeed(seed);

    if (prg_mode_ == PRG_MODE::kNistAesCtrDrbg) {
      ctr_drbg_ = std::make_unique<yasl::crypto::NistAesDrbg>(seed);
    } else if (prg_mode_ == PRG_MODE::kGmSm4CtrDrbg) {
      ctr_drbg_ = std::make_unique<yasl::crypto::Sm4Drbg>(seed);
    }
  }
 ...
 private:
  ...
  std::unique_ptr<yasl::crypto::IDrbg> ctr_drbg_;
};
