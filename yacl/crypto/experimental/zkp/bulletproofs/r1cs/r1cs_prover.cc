...
// ===================================
R1CSProver::R1CSProver(SimpleTranscript* transcript, const PedersenGens* pc_gens)
...
Result<Variable, R1CSError> R1CSProver::Allocate(std::optional<yacl::math::MPInt> assignment) {
  if (!assignment.has_value()) {
    return Result<Variable, R1CSError>::Err(R1CSError(R1CSError::Code::MissingAssignment));
  }
...
Result<std::tuple<Variable, Variable, Variable>, R1CSError> R1CSProver::AllocateMultiplier(
  ...
  return Result<std::tuple<Variable, Variable, Variable>, R1CSError>::Ok(
      {Variable::MultiplierLeft(i), Variable::MultiplierRight(i), Variable::MultiplierOutput(i)});
}
...
void R1CSProver::Constrain(LinearCombination lc) { constraints_.push_back(std::move(lc)); }

Result<void> R1CSProver::SpecifyRandomizedConstraints(RandomizedCallback callback) {
...
yacl::math::MPInt R1CSProver::Eval(const LinearCombination& lc) const {
  ...
  for (const auto& term : lc.terms) {
    ...
    switch (term.first.type) {
      case VariableType::Committed: var_value = v_[term.first.index]; break;
      case VariableType::MultiplierLeft: var_value = a_L_[term.first.index]; break;
      case VariableType::MultiplierRight: var_value = a_R_[term.first.index]; break;
      case VariableType::MultiplierOutput: var_value = a_O_[term.first.index]; break;
      case VariableType::One: var_value = yacl::math::MPInt(1); break;
    }
...
R1CSProver::FlattenedConstraints(const yacl::math::MPInt& z) const {
    ...
    for (const auto& lc : constraints_) {
        for (const auto& term : lc.terms) {
            switch(term.first.type) {
                case VariableType::MultiplierLeft: wL[term.first.index] += exp_z * term.second; break;
                case VariableType::MultiplierRight: wR[term.first.index] += exp_z * term.second; break;
                case VariableType::MultiplierOutput: wO[term.first.index] += exp_z * term.second; break;
                case VariableType::Committed: wV[term.first.index] -= exp_z * term.second; break;
                case VariableType::One: break; // Prover ignores constant term
            }
        }
        exp_z *= z;
    }
...
Result<R1CSProof, R1CSError> R1CSProver::Prove(const BulletproofGens* bp_gens) const {
  ...
  YACL_ENFORCE(bp_gens->gens_capacity() >= n1, "Invalid generators length for phase 1");
...
if (n2 > 0) {
    ...
    s_L2.reserve(n2); s_R2.reserve(n2);
  ...
  yacl::math::MPInt t_2_blinding = InnerProduct(absl::MakeSpan(wV), absl::MakeSpan(v_blinding_), curve);
  ...
  yacl::math::MPInt e_blinding = x * (i_blinding + x * (o_blinding + x * s_blinding));
  ...
  for(size_t i = n1; i < padded_n; ++i) { G_factors_ipp[i] = u; }
  ...
  auto ipp_proof = InnerProductProof::Create(
      *transcript_, curve, Q_ipp,
      G_factors_ipp, H_factors_ipp,
      gens_share.G(padded_n), gens_share.H(padded_n),
      std::move(l_vec), std::move(r_vec)
  );

  return Result<R1CSProof, R1CSError>::Ok(R1CSProof{
      A_I1, A_O1, S1, A_I2, A_O2, S2, T_1, T_3, T_4, T_5, T_6,
