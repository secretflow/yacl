...
Result<std::tuple<Variable, Variable, Variable>, R1CSError> R1CSVerifier::AllocateMultiplier(
...
Result<void, R1CSError> R1CSVerifier::SpecifyRandomizedConstraints(RandomizedCallback callback) {
    ...
    for (const auto& lc : constraints_) {
        for (const auto& term : lc.terms) {
            switch(term.first.type) {
                ...
                case VariableType::One: wc -= exp_z * term.second; break;
            }
        }
        exp_z *= z;
    }
...
Result<void, R1CSError> R1CSVerifier::Verify(const R1CSProof& proof,
  ...
  auto ipp_scalars = proof.ipp_proof.VerificationScalars(padded_n, *transcript_, curve);
  ...
  size_t msm_size = 13 + V_.size() + (2 * padded_n) + u_sq.size() + u_inv_sq.size();
  ...
  if (!curve->IsInfinity(mega_check_point)) {
    return Result<void, R1CSError>::Err(R1CSError(R1CSError::Code::VerificationError));
  }
