...
class R1CSVerifier : public ConstraintSystem {
 public:
  R1CSVerifier(SimpleTranscript* transcript,
  ...
  // Randomization API
  using RandomizedCallback =
      std::function<Result<void, R1CSError>(RandomizedVerifier*)>;
  Result<void, R1CSError> SpecifyRandomizedConstraints(
      RandomizedCallback callback);

  // Proof verification
  Result<void, R1CSError> Verify(const R1CSProof& proof,
