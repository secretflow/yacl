...
void TestStage6SignConstructorRejectsInvalidKeygenProofArtifacts() {
  ...
  if (square_it == bad_square_cfg.all_square_free_proofs.end() ||
      square_it->second.blob.empty()) {
    throw std::runtime_error(
        "test setup missing square-free proof blob to tamper");
  }
  square_it->second.blob.back() ^= 0x01;
  ExpectThrow(
      [&]() { (void)SignSession(std::move(bad_square_cfg)); },
  ...
  if (aux_it == bad_aux_cfg.all_aux_param_proofs.end() ||
      aux_it->second.blob.empty()) {
...
void TestStage6MalformedPhase2InitProofPayloadAbortsResponder() {
  ...
  const SignFixture fixture = BuildSignFixture(signers);
  auto sessions =
      BuildSignSessions(fixture, keygen_results, Bytes{0xF0, 0x02, 0x01});
  ...
  for (Envelope& envelope : phase2_init) {
    if (envelope.type == SignSession::MessageTypeForPhase(SignPhase::kPhase2) &&
        envelope.from == 1 && envelope.to == 2 && envelope.payload.size() > 1) {
  ...
  Expect(tampered,
         "Test setup failed to locate phase2 init payload to truncate");
  ...
  for (const auto& session : sessions) {
    Expect(!session->HasResult(),
           "Malformed phase2 init payload must not expose signature result");
  }
}
...
void TestStage6MalformedPhase2ResponseProofPayloadAbortsInitiator() {
  ...
  const SignFixture fixture = BuildSignFixture(signers);
  auto sessions =
      BuildSignSessions(fixture, keygen_results, Bytes{0xF1, 0x02, 0x01});

  DeliverSignEnvelopesOrThrow(CollectPhase1Messages(&sessions), signers,
                              &sessions);
  EnsureAllSessionsInPhase(sessions, SignPhase::kPhase2);
  DeliverSignEnvelopesOrThrow(CollectPhase2Messages(&sessions), signers,
                              &sessions);
  ...
  for (Envelope& envelope : phase2_responses) {
    if (envelope.type == SignSession::Phase2ResponseMessageType() &&
        envelope.from == 2 && envelope.to == 1 && envelope.payload.size() > 1) {
      envelope.payload.resize(envelope.payload.size() - 1);
      tampered = true;
      break;
    }
  ...
  Expect(tampered,
         "Test setup failed to locate phase2 response payload to truncate");
  ...
  for (const auto& session : sessions) {
    Expect(
        !session->HasResult(),
        "Malformed phase2 response payload must not expose signature result");
  }
}
...
void TestStage4Phase2InitUsesResponderOwnedAuxParams() {
  ...
  responder_cfg.all_aux_param_proofs[2] = tecdsa::BuildAuxRsaParamProof(
  ...
  const size_t responder_idx = FindPartyIndexOrThrow(signers, 2);
  Expect(sessions[responder_idx]->status() == SessionStatus::kAborted,
         "Responder must abort when A1 verification uses responder-owned aux "
         "params");
  for (const auto& session : sessions) {
    Expect(!session->HasResult(),
           "Stage4 A1 ownership failure must not expose signature result");
  }
}
...
void TestStage4Phase2ResponseUsesInitiatorOwnedAuxParams() {
  ...
  responder_cfg.all_aux_param_proofs[1] = tecdsa::BuildAuxRsaParamProof(
      self_aux_it->second,
      BuildKeygenProofContext(responder_cfg.keygen_session_id,
                              /*prover_id=*/1));
  ...
  std::vector<Envelope> initiator_msgs =
      sessions[initiator_idx]->BuildPhase2MtaEnvelopes();
  ...
  Expect(delivered_init_count > 0,
         "Test setup failed to deliver any phase2 init envelope");

  std::vector<Envelope> responder_msgs =
      sessions[responder_idx]->BuildPhase2MtaEnvelopes();
  std::vector<Envelope> responder_responses;
  for (const Envelope& envelope : responder_msgs) {
    if (envelope.type == SignSession::Phase2ResponseMessageType() &&
        envelope.from == 2 && envelope.to == 1) {
  ...
  Expect(sessions[initiator_idx]->status() == SessionStatus::kAborted,
         "Initiator must abort when A2/A3 verification uses initiator-owned "
         "aux params");
  for (const auto& session : sessions) {
    Expect(!session->HasResult(),
           "Stage4 A2/A3 ownership failure must not expose signature result");
  }
}
...
void TestM4SignEndToEndProducesVerifiableSignature() {
  ...
  const SignFixture fixture = BuildSignFixture(signers);
  auto sessions =
      BuildSignSessions(fixture, keygen_results, Bytes{0xE1, 0x02, 0x01});

  RunToPhase5D(&sessions, signers);

  DeliverSignEnvelopesOrThrow(CollectPhase5DMessages(&sessions), signers,
                              &sessions);
  EnsureAllSessionsInPhase(sessions, SignPhase::kPhase5,
                           SignPhase5Stage::kPhase5E);

  DeliverSignEnvelopesOrThrow(CollectPhase5EMessages(&sessions), signers,
                              &sessions);
...
void TestM4Phase5DFailurePreventsPhase5EReveal() {
  ...
  auto sessions =
      BuildSignSessions(bad_fixture, keygen_results, Bytes{0xE2, 0x02, 0x01});
  ...
  Expect(
      any_aborted,
      "At least one party must abort at phase5D when open payload is tampered");
  ...
  for (auto& session : sessions) {
    if (session->status() == SessionStatus::kAborted) {
      ...
      Expect(session->status() == SessionStatus::kRunning,
             "Non-aborted peers should remain running after remote phase5D "
             "failure");
      Expect(session->phase() == SignPhase::kPhase5,
    ...
    Expect(!session->HasResult(),
           "Failure path must not expose final signature");
  }
}
...
void TestM5Phase2InstanceIdMismatchAborts() {
  ...
  const SignFixture fixture = BuildSignFixture(signers);
  auto sessions =
      BuildSignSessions(fixture, keygen_results, Bytes{0xE3, 0x02, 0x01});
  ...
  Expect(tampered,
         "Test setup failed to locate a phase2 response envelope to tamper");
  ...
  for (const auto& session : sessions) {
    ...
    Expect(!session->HasResult(),
           "Phase2-aborted session must not expose final signature");
  }
  Expect(any_aborted,
         "At least one signer must abort on mismatched phase2 instance id");
}
...
void TestM7TamperedPhase2A1ProofAbortsResponder() {
  ...
  const SignFixture fixture = BuildSignFixture(signers);
  auto sessions =
      BuildSignSessions(fixture, keygen_results, Bytes{0xE7, 0x02, 0x01});
  ...
  for (Envelope& envelope : phase2_init) {
    if (envelope.type == SignSession::MessageTypeForPhase(SignPhase::kPhase2) &&
        envelope.from == 1 && envelope.to == 2 && !envelope.payload.empty()) {
  ...
  Expect(tampered,
         "Test setup failed to locate phase2 init A1 proof payload to tamper");
  ...
  for (const auto& session : sessions) {
    Expect(!session->HasResult(),
           "Phase2 A1 proof failure must not expose signature");
  }
}
...
void TestM7TamperedPhase2A3ProofAbortsInitiator() {
  ...
  const SignFixture fixture = BuildSignFixture(signers);
  auto sessions =
      BuildSignSessions(fixture, keygen_results, Bytes{0xE8, 0x02, 0x01});
  ...
  for (Envelope& envelope : phase2_responses) {
    if (envelope.type != SignSession::Phase2ResponseMessageType() ||
        envelope.from != 2 || envelope.to != 1 || envelope.payload.size() < 4) {
      continue;
    }
  ...
  Expect(
      tampered,
      "Test setup failed to locate phase2 response A3 proof payload to tamper");
  ...
  for (const auto& session : sessions) {
    Expect(!session->HasResult(),
           "Phase2 A3 proof failure must not expose signature");
  }
}
...
void TestM7TamperedPhase2A2ProofAbortsInitiator() {
  ...
  const SignFixture fixture = BuildSignFixture(signers);
  auto sessions =
      BuildSignSessions(fixture, keygen_results, Bytes{0xE9, 0x02, 0x01});
  ...
  Expect(
      tampered,
      "Test setup failed to locate phase2 response A2 proof payload to tamper");
  ...
  for (const auto& session : sessions) {
    Expect(!session->HasResult(),
           "Phase2 A2 proof failure must not expose signature");
  }
}
...
void TestM6TamperedPhase4GammaSchnorrAbortsReceiver() {
  ...
  const SignFixture fixture = BuildSignFixture(signers);
  auto sessions =
      BuildSignSessions(fixture, keygen_results, Bytes{0xE4, 0x02, 0x01});
  ...
  Expect(tampered,
         "Test setup failed to locate phase4 proof payload to tamper");
  ...
  for (const auto& session : sessions) {
    Expect(!session->HasResult(),
           "Phase4 proof failure must not expose signature result");
  }
}
...
void TestM6TamperedPhase5BASchnorrAbortsReceiver() {
  ...
  const SignFixture fixture = BuildSignFixture(signers);
  auto sessions =
      BuildSignSessions(fixture, keygen_results, Bytes{0xE5, 0x02, 0x01});
  ...
  Expect(tampered,
         "Test setup failed to locate phase5B Schnorr proof payload to tamper");
  ...
  for (const auto& session : sessions) {
    Expect(!session->HasResult(),
           "Phase5B Schnorr proof failure must not expose signature result");
  }
}
...
void TestM6TamperedPhase5BVRelationAbortsReceiver() {
  ...
  const SignFixture fixture = BuildSignFixture(signers);
  auto sessions =
      BuildSignSessions(fixture, keygen_results, Bytes{0xE6, 0x02, 0x01});
  ...
  Expect(
      tampered,
      "Test setup failed to locate phase5B relation proof payload to tamper");
  ...
  for (const auto& session : sessions) {
    Expect(!session->HasResult(),
           "Phase5B relation proof failure must not expose signature result");
  }
}
...
void TestM9TamperedPhase4GammaPointAbortsReceiver() {
  ...
  const SignFixture fixture = BuildSignFixture(signers);
  auto sessions =
      BuildSignSessions(fixture, keygen_results, Bytes{0xEA, 0x02, 0x01});
  ...
  for (const Envelope& envelope : phase4_messages) {
    if (envelope.from == 2 && envelope.payload.size() >= 33) {
      replacement_gamma.assign(envelope.payload.begin(),
                               envelope.payload.begin() + 33);
      break;
    }
  }
  Expect(!replacement_gamma.empty(),
         "Test setup failed to capture replacement Gamma_i point");
  ...
  Expect(tampered,
         "Test setup failed to locate phase4 Gamma_i payload to tamper");
  ...
  for (const auto& session : sessions) {
    Expect(!session->HasResult(),
           "Tampered Gamma_i path must not expose signature result");
  }
}
...
void TestM9TamperedPhase5ACommitmentAbortsReceiver() {
  ...
  const SignFixture fixture = BuildSignFixture(signers);
  auto sessions =
      BuildSignSessions(fixture, keygen_results, Bytes{0xEB, 0x02, 0x01});
  ...
  Expect(tampered,
         "Test setup failed to locate phase5A commitment payload to tamper");
  ...
  for (const auto& session : sessions) {
    Expect(!session->HasResult(),
           "Tampered phase5A commitment path must not expose signature result");
  }
}
...
void TestM9TamperedPhase3DeltaShareAbortsAndNoResult() {
  ...
  const SignFixture fixture = BuildSignFixture(signers);
  auto sessions =
      BuildSignSessions(fixture, keygen_results, Bytes{0xEC, 0x02, 0x01});
  ...
  for (const auto& session : sessions) {
    ...
    Expect(!session->HasResult(),
           "Tampered delta path must not expose signature result");
  }
  Expect(any_aborted,
         "At least one party must abort when phase3 delta share is tampered");
}
...
void TestM9TamperedPhase5BVPointAbortsReceiver() {
  ...
  const SignFixture fixture = BuildSignFixture(signers);
  auto sessions =
      BuildSignSessions(fixture, keygen_results, Bytes{0xED, 0x02, 0x01});
  ...
  for (const Envelope& envelope : phase5b_messages) {
    if (envelope.from == 2 && envelope.payload.size() >= 33) {
      replacement_v.assign(envelope.payload.begin(),
                           envelope.payload.begin() + 33);
      break;
    }
  }
  Expect(!replacement_v.empty(),
         "Test setup failed to capture replacement V_i point");
  ...
  for (const auto& session : sessions) {
    Expect(!session->HasResult(),
           "Tampered V_i path must not expose signature result");
  }
}
