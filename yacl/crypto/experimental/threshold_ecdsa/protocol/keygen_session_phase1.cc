...
Envelope KeygenSession::BuildPhase1CommitEnvelope() {
  if (IsTerminal()) {
    TECDSA_THROW_LOGIC(
        "cannot build phase1 envelope for terminal keygen session");
  }
  if (phase_ != KeygenPhase::kPhase1) {
    TECDSA_THROW_LOGIC(
        "BuildPhase1CommitEnvelope must be called in keygen phase1");
  }
  ...
  payload.insert(payload.end(), local_commitment_.begin(),
                 local_commitment_.end());
  ...
  const Bytes aux_param_proof_wire =
      EncodeAuxRsaParamProof(local_aux_param_proof_);
...
void KeygenSession::EnsureLocalPaillierPrepared() {
  ...
  for (size_t attempt = 0; attempt < ki::kMaxPaillierKeygenAttempts;
       ++attempt) {
...
void KeygenSession::EnsureLocalStrictProofArtifactsPrepared() {
  ...
  if (local_paillier_ == nullptr) {
    TECDSA_THROW_LOGIC(
        "local Paillier key must be prepared before strict artifacts");
  }

  const StrictProofVerifierContext context =
      ki::BuildStrictProofContext(session_id(), self_id());
  local_aux_rsa_params_ =
      GenerateAuxRsaParams(aux_rsa_modulus_bits_, self_id());
  ...
  if (require_aux_param_proof_) {
    local_aux_param_proof_ =
        BuildAuxRsaParamProof(local_aux_rsa_params_, context);
  } else {
  ...
  result_.aux_param_proof_profile = require_aux_param_proof_
  ...
  if (strict_mode_) {
    if (!ki::StrictMetadataCompatible(expected_square_free_proof_profile_,
      ...
      TECDSA_THROW(
          "local square-free proof metadata does not match expected strict "
          "profile");
    }
    ...
    if (require_aux_param_proof_) {
      if (!ki::StrictMetadataCompatible(expected_aux_param_proof_profile_,
        ...
        TECDSA_THROW(
            "local aux proof metadata does not match expected strict profile");
      }
      if (!VerifyAuxRsaParamProofStrict(local_aux_rsa_params_,
  ...
  try {
    ...
    Bytes commitment(envelope.payload.begin(),
                     envelope.payload.begin() +
                         static_cast<std::ptrdiff_t>(ki::kCommitmentLen));
    offset += ki::kCommitmentLen;

    const BigInt paillier_n = ki::ReadMpIntField(
        envelope.payload, &offset, ki::kMaxPaillierModulusFieldLen,
        "keygen phase1 Paillier modulus");
    ...
    if (offset == envelope.payload.size()) {
      if (strict_mode_) {
        TECDSA_THROW_ARGUMENT(
            "legacy phase1 payload shape is not allowed in strict mode");
      }
      aux_params = DeriveAuxRsaParamsFromModulus(pub.n, envelope.from);
    } else {
      aux_params.n_tilde = ki::ReadMpIntField(envelope.payload, &offset,
                                              ki::kMaxPaillierModulusFieldLen,
                                              "keygen phase1 aux Ntilde");
      aux_params.h1 = ki::ReadMpIntField(envelope.payload, &offset,
                                         ki::kMaxPaillierModulusFieldLen,
                                         "keygen phase1 aux h1");
      aux_params.h2 = ki::ReadMpIntField(envelope.payload, &offset,
                                         ki::kMaxPaillierModulusFieldLen,
                                         "keygen phase1 aux h2");
      ...
      const Bytes aux_param_proof_wire =
          ki::ReadSizedField(envelope.payload, &offset, ki::kMaxProofFieldLen,
                             "keygen phase1 aux parameter proof");
      if (!aux_param_proof_wire.empty()) {
        aux_param_proof =
            DecodeAuxRsaParamProof(aux_param_proof_wire, ki::kMaxProofBlobLen);
...
void KeygenSession::MaybeAdvanceAfterPhase1() {
  ...
  if (strict_mode_) {
    ...
    if (require_aux_param_proof_) {
      if (result_.aux_param_proof_profile.scheme ==
          StrictProofScheme::kUnknown) {
        return;
      }
