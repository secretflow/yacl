...
FpPolynomial FpPolynomial::Mul(const FpPolynomial& g) const {
  ...
  if (wide_ntt) {
    ...
    for (std::size_t k = 0; k < prime_count; ++k) {
      std::vector<detail::u64> a64(n);
      std::vector<detail::u64> b64(m);
    ...
    for (std::size_t k = 0; k < prime_count; ++k) {
      ...
      std::vector<detail::u32> a32(n);
      std::vector<detail::u32> b32(m);
      // [BARRETT INPUT REDUCE] avoid % in input conversion
      auto& CC = detail::ensure_ntt_cache(
          k, 1U);  // only to access barrett_im (no rebuild)
      const detail::u32 mod = prm.mod;
      const detail::u64 im = CC.barrett_im;

      for (std::size_t i = 0; i < n; ++i) {
        a32[i] = detail::barrett_reduce_u64(static_cast<detail::u64>(c_[i].v),
                                            mod, im);
      }
      for (std::size_t j = 0; j < m; ++j) {
        b32[j] = detail::barrett_reduce_u64(static_cast<detail::u64>(g.c_[j].v),
                                            mod, im);
      }

      // [BUFFER REUSE] in-place convolution: residues[k] is fa (work +
      // output), fb is thread_local scratch inside convolution_mod_ntt_into
...
FpPolynomial FpPolynomial::MulTruncPoly(const FpPolynomial& a,
  ...
  if (wide_ntt) {
    ...
    std::vector<std::vector<detail::u32>> residues;
    ...
    for (std::size_t idxp = 0; idxp < prime_count; ++idxp) {
      ...
      for (size_type i = 0; i < an; ++i) {
        A32[i] = detail::barrett_reduce_u64(static_cast<detail::u64>(a.c_[i].v),
                                            mod, im);
      }
      for (size_type j = 0; j < bn; ++j) {
        B32[j] = detail::barrett_reduce_u64(static_cast<detail::u64>(b.c_[j].v),
                                            mod, im);
      }
