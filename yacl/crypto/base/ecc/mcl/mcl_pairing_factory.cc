...
#endif
    default:
      YACL_THROW("Not supported pairing!");
  }
  return Create(meta);
}
...
#define CASE_DEFINE(mcl_pairing_macro, class_name, namespace_name)           \
  case MCL_##mcl_pairing_macro: {                                            \
    ...
    auto g1 = std::unique_ptr<EcGroup>(                                      \
        new MclPairing##class_name##G1(meta, pairing_type, AnyPtr(p1)));     \
    ...
    auto g2 = std::unique_ptr<EcGroup>(                                      \
        new MclPairing##class_name##G2(meta, pairing_type, AnyPtr(p2)));     \
...
std::unique_ptr<PairingGroup> MclPGFactory::Create(const PairingMeta& meta) {
  ...
  switch (pairing_type) {
    ...
    case MCL_BLS12_381: {
      ...
      auto g1 = std::unique_ptr<EcGroup>(
          new MclPairingBls12381G1(meta, pairing_type, AnyPtr(p1)));
      ...
      // Init G2
      auto g2 = std::unique_ptr<EcGroup>(
          new MclPairingBls12381G2(meta, pairing_type, AnyPtr(p2)));
...
#endif
    default:
      YACL_THROW("Not supported pairing in {}!", kLibName);
  }
}
