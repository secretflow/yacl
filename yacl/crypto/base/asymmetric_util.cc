...
inline std::string BioToString(const UniqueBio& bio) {
  ...
  YACL_ENFORCE_GT(size, 0, "BIO_pending failed.");
  std::string out;
  out.resize(size);
  YACL_ENFORCE_EQ(BIO_read(bio.get(), out.data(), size), size,
                  "Read bio failed.");
  return out;
}
...
UniquePkey CreatePriPkeyFromSm2Pem(ByteContainerView pem) {
  ...
  YACL_ENFORCE(ec_key != nullptr, "No ec private key from pem.");
  ON_SCOPE_EXIT([&] { EC_KEY_free(ec_key); });
  EVP_PKEY* pri_key = EVP_PKEY_new();
  YACL_ENFORCE(pri_key != nullptr);
  YACL_ENFORCE_GT(EVP_PKEY_set1_EC_KEY(pri_key, ec_key), 0);
  YACL_ENFORCE_GT(EVP_PKEY_set_alias_type(pri_key, EVP_PKEY_SM2), 0);
...
UniquePkey CreatePubPkeyFromSm2Pem(ByteContainerView pem) {
  ...
  YACL_ENFORCE(ec_key != nullptr, "No ec public key from pem.");
  ON_SCOPE_EXIT([&] { EC_KEY_free(ec_key); });
  EVP_PKEY* pub_key = EVP_PKEY_new();
  YACL_ENFORCE(pub_key != nullptr);
  YACL_ENFORCE_GT(EVP_PKEY_set1_EC_KEY(pub_key, ec_key), 0);
  YACL_ENFORCE_GT(EVP_PKEY_set_alias_type(pub_key, EVP_PKEY_SM2), 0);
...
std::tuple<std::string, std::string> CreateSm2KeyPair() {
  ...
  YACL_ENFORCE(ec_key != nullptr);
  ON_SCOPE_EXIT([&] { EC_KEY_free(ec_key); });
  EC_GROUP* ec_group = EC_GROUP_new_by_curve_name(NID_sm2);
  YACL_ENFORCE(ec_group != nullptr);
  ON_SCOPE_EXIT([&] { EC_GROUP_free(ec_group); });
  YACL_ENFORCE_GT(EC_KEY_set_group(ec_key, ec_group), 0);
  YACL_ENFORCE_GT(EC_KEY_generate_key(ec_key), 0);

  // Read private key
  BIO* pri_bio = BIO_new(BIO_s_mem());
  ON_SCOPE_EXIT([&] { BIO_free(pri_bio); });
  YACL_ENFORCE_GT(PEM_write_bio_ECPrivateKey(pri_bio, ec_key, nullptr, nullptr,
                                             0, nullptr, nullptr),
                  0);
  std::string private_key(BIO_pending(pri_bio), '\0');
  YACL_ENFORCE_GT(BIO_read(pri_bio, private_key.data(), private_key.size()), 0);

  // Read public key
  BIO* pub_bio = BIO_new(BIO_s_mem());
  ON_SCOPE_EXIT([&] { BIO_free(pub_bio); });
  YACL_ENFORCE_GT(PEM_write_bio_EC_PUBKEY(pub_bio, ec_key), 0);
  std::string public_key(BIO_pending(pub_bio), '\0');
  YACL_ENFORCE_GT(BIO_read(pub_bio, public_key.data(), public_key.size()), 0);
...
std::string GetPublicKeyFromRsa(const UniqueRsa& rsa, bool x509_pkey) {
  std::string public_key;
  {
    ...
    YACL_ENFORCE(bio, "New bio failed.");
    if (x509_pkey) {
      UniqueEVP unique_pkey(EVP_PKEY_new(), ::EVP_PKEY_free);
      YACL_ENFORCE(EVP_PKEY_set1_RSA(unique_pkey.get(), rsa.get()),
                   "Convert rsa to pubkey failed.");
      YACL_ENFORCE(PEM_write_bio_PUBKEY(bio.get(), unique_pkey.get()),
                   "Write public key failed.");
    } else {
      YACL_ENFORCE(PEM_write_bio_RSAPublicKey(bio.get(), rsa.get()),
                   "Write public key failed.");
    }
    int size = BIO_pending(bio.get());
    YACL_ENFORCE_GT(size, 0, "Bad key size.");
    public_key.resize(size);
    YACL_ENFORCE_GT(BIO_read(bio.get(), public_key.data(), size), 0,
                    "Cannot read bio.");
  }
  return public_key;
}
...
std::tuple<std::string, std::string> CreateRsaKeyPair(bool x509_pkey) {
  ...
  YACL_ENFORCE_EQ(BN_set_word(exp.get(), RSA_F4), 1, "BN_set_word failed.");
  ...
  std::string private_key;
  {
    UniqueBio bio(BIO_new(BIO_s_mem()), BIO_free);
    YACL_ENFORCE(bio, "New bio failed.");
    YACL_ENFORCE(PEM_write_bio_RSAPrivateKey(bio.get(), rsa.get(), nullptr,
                                             nullptr, 0, 0, nullptr),
                 "Write private key failed.");
    int size = BIO_pending(bio.get());
    YACL_ENFORCE_GT(size, 0, "Bad key size.");
    private_key.resize(size);
    YACL_ENFORCE_GT(BIO_read(bio.get(), private_key.data(), size), 0,
                    "Cannot read bio.");
  }
...
std::tuple<std::string, std::string> CreateRsaCertificateAndPrivateKey(
  ...
  YACL_ENFORCE_EQ(BN_set_word(exp.get(), RSA_F4), 1, "BN_set_word failed.");
  UniqueRsa rsa(RSA_new(), ::RSA_free);
  YACL_ENFORCE(RSA_generate_key_ex(rsa.get(), bit_length, exp.get(), nullptr),
               "Generate rsa key pair failed.");

  // 2. Assign to EVP_PKEY.
  UniqueEVP evp_pkey(EVP_PKEY_new(), ::EVP_PKEY_free);
  YACL_ENFORCE(EVP_PKEY_assign_RSA(evp_pkey.get(), rsa.get()),
               "Cannot assign rsa.");
  ...
  YACL_ENFORCE(ASN1_INTEGER_set(X509_get_serialNumber(x509.get()), rd()) == 1,
  ...
  for (auto& field : kSubjectFields) {
    ...
    YACL_ENFORCE(it != subject_map.end(), "Cannot find subject field {}.",
                 field);
  ...
  YACL_ENFORCE(X509_sign(x509.get(), evp_pkey.get(), EVP_sha256()),
               "Perform self-signing failed.");
  // 4. Write as string.
  UniqueBio pkey_bio(BIO_new(BIO_s_mem()), BIO_free);
  YACL_ENFORCE(PEM_write_bio_PrivateKey(pkey_bio.get(), evp_pkey.get(), nullptr,
                                        nullptr, 0, nullptr, nullptr),
               "Failed PEM_write_bio_PrivateKey.");
  UniqueBio cert_bio(BIO_new(BIO_s_mem()), BIO_free);
  YACL_ENFORCE(PEM_write_bio_X509(cert_bio.get(), x509.get()),
