...
TEST(SymmetricCrypto, FillAesRandom) {
  std::string no_padding(SymmetricCrypto::BlockSize() * 16, 0);
  std::string need_padding(SymmetricCrypto::BlockSize() * 15 + 2, 0);

  uint128_t zero_key = 0;
  uint128_t zero_iv = 0;
  uint64_t count = 0;

  auto count1 = FillPseudoRandom(
      SymmetricCrypto::CryptoType::AES128_ECB, zero_key, zero_iv, count,
      absl::MakeSpan(static_cast<char*>(no_padding.data()), no_padding.size()));
  auto count2 = FillPseudoRandom(
      SymmetricCrypto::CryptoType::AES128_ECB, zero_key, zero_iv, count,
      absl::MakeSpan(static_cast<char*>(need_padding.data()),
                     need_padding.size()));
  auto count3 = DummyUpdateRandomCount(
      count, absl::MakeSpan(static_cast<char*>(need_padding.data()),
                            need_padding.size()));
...
TEST(SymmetricCrypto, FillSM4Random) {
  std::string no_padding(SymmetricCrypto::BlockSize() * 16, 0);
  std::string need_padding(SymmetricCrypto::BlockSize() * 15 + 2, 0);
  ...
  auto count1 = FillPseudoRandom(
      SymmetricCrypto::CryptoType::SM4_ECB, zero_key, zero_iv, count,
      absl::MakeSpan(static_cast<char*>(no_padding.data()), no_padding.size()));
  auto count2 = FillPseudoRandom(
      SymmetricCrypto::CryptoType::SM4_ECB, zero_key, zero_iv, count,
      absl::MakeSpan(static_cast<char*>(need_padding.data()),
