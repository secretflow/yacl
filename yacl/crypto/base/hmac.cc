...
#include "yacl/crypto/base/hmac.h"
...
Hmac& Hmac::Update(ByteContainerView data) {
  YACL_ENFORCE(HMAC_Update(context_, data.data(), data.size()) == 1,
               "HMAC_Update failed");
  return *this;
}
...
std::vector<uint8_t> Hmac::CumulativeMac() const {
  ...
  YACL_ENFORCE_EQ(HMAC_CTX_copy(context_snapshot, context_), 1);
  size_t mac_size = HMAC_size(context_snapshot);
  YACL_ENFORCE_GT(mac_size, (size_t)0);
  std::vector<uint8_t> mac(mac_size);
  unsigned int len;
  YACL_ENFORCE_EQ(HMAC_Final(context_snapshot, mac.data(), &len), 1);
