  ...
  void TestOrder() {
    ...
    if (field_->GetExtensionDegree() == 1) {
      ...
      if (!field_->GetAddGroupOrder().IsZero()) {
        ...
        // TODO: mpint 2 Fp
        // order_fp.setMpz(crypto::hmcl::Mp2Mpz(order));
...
#define DECLARE_PAIRING_FIELD_TEST_CLASS(classname, pairing_name)            \
  class MclPairing##classname##GTTest                                        \
      : public MclFieldTest<crypto::hmcl::MclPairing##classname##GT> {       \
    void SetUp() override {                                                  \
      auto pairing = crypto::hmcl::MclPGFactory::CreateByName(pairing_name); \
