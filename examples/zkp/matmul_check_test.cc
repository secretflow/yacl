...
public:
    DenseMultilinearPolynomial(std::vector<FieldElem> evaluations, FieldElem modulus)
        ...
        YACL_ENFORCE(evaluations_.size() > 0 && (evaluations_.size() & (evaluations_.size() - 1)) == 0,
    ...
    FieldElem evaluate(const std::vector<FieldElem>& point) const override {
        YACL_ENFORCE(point.size() == num_vars_, "Evaluation point has incorrect number of variables.");
