...
FieldElem RandFieldElem(const FieldElem& modulus) {
    ...
    rand_val.FromMagBytes(absl::MakeConstSpan(rand_bytes.data(), rand_bytes.size()));
    FieldElem::Mod(rand_val, modulus, &rand_val);
    return rand_val;
}

FieldElem SumOverBooleanHypercube(const MultiLinearPolynomialVec& evals, const FieldElem& modulus) {
...
FieldElem MultilinearPolynomial::Evaluate(const std::vector<FieldElem>& r, const FieldElem& modulus) const {
    YACL_ENFORCE(r.size() == num_vars_, "Evaluation point has incorrect number of variables.");
    if (num_vars_ == 0) {
        return evals_[0];
    }
...
LogUpProver::LogUpProver(std::shared_ptr<const MultilinearPolynomial> f_A,
    ...
    : f_A_(std::move(f_A)),
      ...
      m_B_(std::move(m_B)),
      modulus_p_(modulus) {
    YACL_ENFORCE(f_B_->NumVars() == m_B_->NumVars(), "f_B and m_B must have the same number of variables.");
}
...
void LogUpProver::Setup(const FieldElem& zeta) {
    ...
    for (const auto& f_A_val : f_A_evals) {
        ...
        YACL_ENFORCE(denominator != zero, "Division by zero in h_A construction: zeta is a root of f_A.");
    ...
    for (size_t i = 0; i < f_B_evals.size(); ++i) {
        ...
        YACL_ENFORCE(denominator != zero, "Division by zero in h_B construction: zeta is a root of f_B for a non-zero m_B entry.");
...
bool LogUpVerifier::Verify(LogUpProver& prover) {
    ...
    if (!RunSumcheckProtocol(h_A->GetEvals(), claimed_sum_A, modulus_p_)) {return false;}
    auto h_B = prover.Get_h_B();
    if (!RunSumcheckProtocol(h_B->GetEvals(), claimed_sum_B, modulus_p_)) {return false;}
    auto q_A = prover.Get_q_A();
    if (!RunZeroCheckProtocol(q_A->GetEvals(), modulus_p_)) {return false;}
    auto q_B = prover.Get_q_B();
    if (!RunZeroCheckProtocol(q_B->GetEvals(), modulus_p_)) {return false;}
    return true;
}
