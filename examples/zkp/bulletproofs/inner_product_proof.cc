...
InnerProductProof InnerProductProof::Create(
    ...
    std::vector<yacl::crypto::EcPoint> L_vec_out; L_vec_out.reserve(lg_n);
    std::vector<yacl::crypto::EcPoint> R_vec_out; R_vec_out.reserve(lg_n);

    while (n > 1) {
        n = n / 2;

        // Create temporary vectors for calculations based on current range [lo, lo + current_n)
        // Scalars
        std::vector<yacl::math::MPInt> a_L(a_vec.begin(), a_vec.begin() + n);
        std::vector<yacl::math::MPInt> a_R(a_vec.begin() + n, a_vec.begin() + 2*n);
        std::vector<yacl::math::MPInt> b_L(b_vec.begin(), b_vec.begin() + n);
        std::vector<yacl::math::MPInt> b_R(b_vec.begin() + n, b_vec.begin() + 2*n);
        // Points
        std::vector<yacl::crypto::EcPoint> G_L(G_vec.begin(), G_vec.begin() + n);
        std::vector<yacl::crypto::EcPoint> G_R(G_vec.begin() + n, G_vec.begin() + 2*n);
        std::vector<yacl::crypto::EcPoint> H_L(H_vec.begin(), H_vec.begin() + n);
        std::vector<yacl::crypto::EcPoint> H_R(H_vec.begin() + n, H_vec.begin() + 2*n);
        // Factors (read only from original vector)
        std::vector<yacl::math::MPInt> G_fact_L(G_factors.begin(), G_factors.begin() + n);
        std::vector<yacl::math::MPInt> G_fact_R(G_factors.begin() + n, G_factors.begin() + 2*n);
        ...
        std::vector<yacl::math::MPInt> L_scalars; L_scalars.reserve(n + n + 1);
        std::vector<yacl::crypto::EcPoint> L_points; L_points.reserve(n + n + 1);
        for (size_t i = 0; i < n; ++i) { L_scalars.push_back(a_L[i]); L_points.push_back(G_R[i]); }
        for (size_t i = 0; i < n; ++i) { L_scalars.push_back(b_R[i]); L_points.push_back(H_L[i]); }
        ...
        std::vector<yacl::math::MPInt> R_scalars; R_scalars.reserve(n + n + 1);
        std::vector<yacl::crypto::EcPoint> R_points; R_points.reserve(n + n + 1);
        for (size_t i = 0; i < n; ++i) { R_scalars.push_back(a_R[i]); R_points.push_back(G_L[i]); }
        for (size_t i = 0; i < n; ++i) { R_scalars.push_back(b_L[i]); R_points.push_back(H_R[i]); }
        ...
        for (size_t i = 0; i < n; ++i) {
            // a = a_L * x + a_R * x_inv
            a_L[i] = a_L[i].MulMod(x, order).AddMod(a_R[i].MulMod(x_inv, order), order);
            // b = b_L * x_inv + b_R * x
...
bool InnerProductProof::Verify(
  ...
  try {
    ...
    std::cout << "Verifying proof with n=" << n << ", lg_n=" << lg_n << std::endl;
    ...
    for(size_t i = 0; i < n; ++i) {
      h_times_b_div_s[i] = H_factors[i].MulMod(b_, order).MulMod(inv_s[i], order);
    }
    ...
    for(size_t i = 0; i < lg_n; ++i) {
      neg_x_inv_sq[i] = neg_x_inv_sq[i].MulMod(neg_x_inv_sq[i], order).MulMod(yacl::math::MPInt(-1), order);
    }
    ...
    msm_scalars.insert(msm_scalars.end(), neg_x_inv_sq.begin(), neg_x_inv_sq.end());
