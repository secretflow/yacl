...
struct BpConfig {
  ...
  bool operator==(const BpConfig& other) const;
};
...
using BpProof = std::vector<yacl::math::MPInt>;
using BpGenerator = std::vector<yacl::crypto::EcPoint>;
using BpStatement = yacl::crypto::EcPoint;

struct BpBatchProof {
  BpProof proof;
  BpStatement rnd_statement;
};
