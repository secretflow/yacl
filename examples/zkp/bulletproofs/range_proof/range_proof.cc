...
// --- Ensure Helpers are Defined (or included from util.cc via util.h) ---
// Placeholder definitions if not in util.cc:
/*
yacl::math::MPInt SumOfPowers(const yacl::math::MPInt& base, size_t count, const std::shared_ptr<yacl::crypto::EcGroup>& curve) { ... }
std::vector<yacl::math::MPInt> ExpIterVector(const yacl::math::MPInt& base, size_t n, const std::shared_ptr<yacl::crypto::EcGroup>& curve) { ... }
yacl::math::MPInt InnerProduct(const std::vector<yacl::math::MPInt>& a, const std::vector<yacl::math::MPInt>& b, const std::shared_ptr<yacl::crypto::EcGroup>& curve) { ... }
std::vector<yacl::math::MPInt> AddVec(const std::vector<yacl::math::MPInt>& a, const std::vector<yacl::math::MPInt>& b, const std::shared_ptr<yacl::crypto::EcGroup>& curve) { ... }
yacl::crypto::EcPoint MultiScalarMul(const std::shared_ptr<yacl::crypto::EcGroup>& curve, const std::vector<yacl::math::MPInt>& scalars, const std::vector<yacl::crypto::EcPoint>& points) { ... }
*/
...
std::vector<yacl::crypto::EcPoint> RangeProof::MakeGenerators(
    ...
    yacl::Buffer base_bytes = curve->SerializePoint(base_point); // true for compressed
    ...
    for (size_t i = 1; i < n; ++i) {
        ...
        // auto hash_val = yacl::crypto::Sha256(yacl::ByteContainerView(prev_bytes));
        // generators[i] = curve->HashToCurve(yacl::crypto::HashToCurveStrategy::Autonomous, // Or appropriate strategy
...
yacl::math::MPInt RangeProof::Delta(
    ...
    yacl::math::MPInt term2 = z.MulMod(zz, order).MulMod(sum_2, order); // z^3 * sum_2
...
RangeProof RangeProof::GenerateProof(
    ...
    // 1. Create Generators
    yacl::crypto::EcPoint B = curve->HashToCurve(yacl::crypto::HashToCurveStrategy::Autonomous, "hello");
    yacl::crypto::EcPoint B_blinding = curve->HashToCurve(yacl::crypto::HashToCurveStrategy::Autonomous, "there");
    std::vector<yacl::crypto::EcPoint> G_vec = MakeGenerators(B, n, curve);
    std::vector<yacl::crypto::EcPoint> H_vec = MakeGenerators(B_blinding, n, curve);
    ...
    yacl::math::MPInt a_blinding; a_blinding.RandomLtN(order, &a_blinding);
    yacl::crypto::EcPoint A = curve->Mul(B_blinding, a_blinding);
    for (size_t i = 0; i < n; ++i) {
        if (((v >> i) & 1) == 0) { A = curve->Sub(A, H_vec[i]); }
        else                    { A = curve->Add(A, G_vec[i]); }
    }

    // 4. Compute S = <s_L, G> + <s_R, H> + s_blinding * B_blinding
    yacl::math::MPInt s_blinding; s_blinding.RandomLtN(order, &s_blinding);
    std::vector<yacl::math::MPInt> s_L(n), s_R(n);
    std::vector<yacl::math::MPInt> s_scalars; s_scalars.reserve(1 + 2*n);
    std::vector<yacl::crypto::EcPoint> s_points; s_points.reserve(1 + 2*n);
    s_scalars.push_back(s_blinding); s_points.push_back(B_blinding);
    for(size_t i = 0; i < n; ++i) {
        s_L[i].RandomLtN(order, &s_L[i]); s_R[i].RandomLtN(order, &s_R[i]);
        s_scalars.push_back(s_L[i]); s_points.push_back(G_vec[i]);
        s_scalars.push_back(s_R[i]); s_points.push_back(H_vec[i]);
    }
    yacl::crypto::EcPoint S = MultiScalarMul(curve, s_scalars, s_points);

    // 5. Commit V, A, S and get challenges y, z
    transcript.AppendPoint("V", V, curve); // Assume AppendPoint handles compression/format consistently
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        r_poly.vec0[i] = r_poly.vec0[i].AddMod(zz.MulMod(two_pows[i], order), order);
        r_poly.vec1[i] = y_pows[i].MulMod(s_R[i], order);
    }

    // 7. Compute t(X) = <l(X), r(X)> (Results in Poly2)
    Poly2 t_poly = l_poly.InnerProduct(r_poly, curve); // t_poly has members t0, t1, t2
    ...
    yacl::crypto::EcPoint T_1 = curve->Add(curve->Mul(B, t_poly.t1), curve->Mul(B_blinding, t_1_blinding));
    yacl::crypto::EcPoint T_2 = curve->Add(curve->Mul(B, t_poly.t2), curve->Mul(B_blinding, t_2_blinding));
    ...
    t_x_blinding = t_x_blinding.AddMod(x.MulMod(x, order).MulMod(t_2_blinding, order), order);
    // e_blinding = a_blinding + x*s_blinding
    yacl::math::MPInt e_blinding = a_blinding.AddMod(x.MulMod(s_blinding, order), order);
    ...
    std::vector<yacl::math::MPInt> ipp_H_factors = ExpIterVector(y_inv, n, curve); // Factors for H_vec
    // IPP vectors a, b
    ...
    return RangeProof(V, A, S, T_1, T_2, t_x, t_x_blinding, e_blinding, ipp_proof);
}
...
bool RangeProof::Verify(
    ...
    // check: V^(z^2) * T1^x * T2^(x^2) * B^(delta - t_x) * B_blinding^(-t_x_blinding) == Identity
    yacl::math::MPInt zz = z.MulMod(z, order);
    ...
    std::vector<yacl::crypto::EcPoint> poly_points = {
        V_, T_1_, T_2_, B, B_blinding
    };
    yacl::crypto::EcPoint poly_check = MultiScalarMul(curve, poly_scalars, poly_points);
    ...
    // 4. IPP Verification
    // Calculate P_final = A + x*S -z*G_sum + <h_prime_scalars, H_prime_vec> + (w*t_x - e_blinding)*B_blinding
    // where H'_i = H_i * y^-i and h_prime_scalar_i = z + zz * 2^i


// Recompute P + t(x)Q = P + t(x)w B_blinding

    yacl::crypto::EcPoint G_sum = curve->MulBase(yacl::math::MPInt(0)); // Identity
    ...
    std::vector<yacl::math::MPInt> exp_two_over_y = ExpIterVector(two_over_y, n, curve);
    std::vector<yacl::math::MPInt> h_scalars; h_scalars.reserve(n);
    for(size_t i=0; i<n; ++i) {
    ...
    yacl::crypto::EcPoint P_plus_tx_Q = MultiScalarMul(curve, msm_scalars, msm_points);
    ...
    std::vector<yacl::math::MPInt> ipp_H_factors = ExpIterVector(y_inv, n, curve); // H factors are y^-i
    ...
    bool ipp_result = ipp_proof_.Verify(n, &transcript, curve,
                             ...
                             P_plus_tx_Q, 
                             Q,
                             G_vec, H_vec); // IPP uses the same G/H vectors
...
yacl::Buffer RangeProof::ToBytes(const std::shared_ptr<yacl::crypto::EcGroup>& curve) const {
    ...
    yacl::Buffer ipp_bytes = ipp_proof_.ToBytes(curve); // Assuming IPP has ToBytes

    size_t header_size = 9 * sizeof(size_t);

    int64_t total_size = header_size +
                        V_bytes.size() + A_bytes.size() + S_bytes.size() +
...
RangeProof RangeProof::FromBytes(
    ...
    InnerProductProof ipp_proof = InnerProductProof::FromBytes(ipp_data, curve); // Assuming IPP has FromBytes
