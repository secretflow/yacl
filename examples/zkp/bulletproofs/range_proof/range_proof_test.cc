...
uint64_t GenerateRandomValueInRange(size_t n, const std::shared_ptr<yacl::crypto::EcGroup>& curve) {
    ...
    if (n - 1 >= 64) { // If n=64 or more (though we restrict n<=64)
        upper_bound_exclusive = std::numeric_limits<uint64_t>::max(); // 2^64 - 1
  ...
  void TestGenerateAndVerify(size_t n) {
    ...
    ASSERT_NO_THROW({
        proof = RangeProof::GenerateProof(
            prover_transcript, curve_, n, v, v_blinding);
    });

    // Verifier side
    SimpleTranscript verifier_transcript("RangeproofTest"); // MUST use same label
    ...
    if (!verification_result) {
        std::cout << "Verification failed for n=" << n << ", v=" << v << std::endl;
        // Optionally serialize proof parts for debugging
        // std::cout << "Proof V: " << curve_->SerializePoint(proof.GetV()) << std::endl;
        // ...
    }

    ASSERT_TRUE(verification_result) << "Verification failed for n=" << n << ", v=" << v;
    ...
    ASSERT_NO_THROW({ deserialized_proof = RangeProof::FromBytes(curve_, proof_bytes); });
    ...
    bool verification_result2 = false;
     ASSERT_NO_THROW({
         verification_result2 = deserialized_proof.Verify(
             verifier_transcript2, curve_, n);
     });
     ASSERT_TRUE(verification_result2) << "Verification failed after serde for n=" << n;
  }
...
TEST_F(RangeProofDirectTest, CreateAndVerify16) {
  TestGenerateAndVerify(16);
}

TEST_F(RangeProofDirectTest, CreateAndVerify32) {
  TestGenerateAndVerify(32);
}

TEST_F(RangeProofDirectTest, CreateAndVerify64) {
  TestGenerateAndVerify(64);
}
