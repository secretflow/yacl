...
class HashMapT1 {
public:
    explicit HashMapT1(std::shared_ptr<yacl::crypto::EcGroup> ec_group, const std::string& filename = "hashmap_t1.dat")
    ...
    HashMapT1(std::shared_ptr<yacl::crypto::EcGroup> ec_group, bool initialize, const std::string& filename = "hashmap_t1.dat")
        : ec_group_(std::move(ec_group)) {
        if (initialize) {
            if (std::filesystem::exists(filename)) {
                Deserialize(filename);
    ...
    void Serialize(const std::string& filename) const {
        ...
        for (const auto& [key, value] : map_) {
            ...
            ofs.write(reinterpret_cast<const char*>(value_bytes.data<uint8_t>()), value_size);
        }
    }
    ...
    void Deserialize(const std::string& filename) {
        ...
        for (size_t i = 0; i < map_size; ++i) {
            ...
            ifs.read(reinterpret_cast<char*>(value_bytes.data<uint8_t>()), value_size);
...
class CuckooT1 {
public:
    explicit CuckooT1(int jmax, std::shared_ptr<yacl::crypto::EcGroup> ec_group = nullptr)
        : jmax_(jmax), cuckoolen_(static_cast<uint32_t>(jmax * 1.3)), ec_group_(std::move(ec_group)) {
        if (jmax_ <= 0) {
    ...
    void Serialize(const std::string& filename) const {
        ...
        ofs.write(reinterpret_cast<const char*>(table_v_.data()), table_v_.size() * sizeof(uint32_t));
        ofs.write(reinterpret_cast<const char*>(table_k_.data()), table_k_.size() * sizeof(uint32_t));
    }
    ...
    void Deserialize(const std::string& filename) {
        ...
        ifs.read(reinterpret_cast<char*>(table_v_.data()), table_v_.size() * sizeof(uint32_t));
        ifs.read(reinterpret_cast<char*>(table_k_.data()), table_k_.size() * sizeof(uint32_t));
    }
